...
use crate::{aggregator::Aggregator, Committee, EpochNumber, Parameters, ViewNumber};
...
impl BinaryAgreement {
    ...
    async fn transmit(&self, msg: BAMessage) -> ConsensusResult<()> {
        transmit(msg, &self.name, None, &self.network_filter, &self.committee).await
    }
    ...
    async fn handle_val(&mut self, val: &BAVote) -> ConsensusResult<()> {
        ...
        let forward_prepared = aggregator.is_verified(
            &self.committee,
            &val.vote,
            &self.committee.random_coin_threshold(),
        );
        let has_qc = aggregator.is_verified(
            &self.committee,
            &val.vote,
            &self.committee.quorum_threshold(),
        );
        let opposite_has_qc = aggregator.is_verified(
            &self.committee,
            &!val.vote,
            &self.committee.quorum_threshold(),
        );
        ...
        if has_qc {
            self.bin_val.entry((val.epoch, val.view)).or_insert({
    ...
    async fn handle_conf(&mut self, conf: &BAConf) -> ConsensusResult<()> {
        ...
        aggregator.append(
            conf.author,
            conf.clone(),
            self.committee.stake(&conf.author),
        )?;
